...
export async function fetchCreditNoteEntriesFromGoogleSheets(config) {
  ...
  if (!response.ok) {
    throw new Error(`Unable to read Credit Note from Google Sheets (${response.status}).`);
  }
  ...
  if (parsed.errors.length) {
    throw new Error(parsed.errors[0].message || "Unable to parse Credit Note CSV.");
  }

  return parseCreditNoteRows(parsed.data);
}
