...
const svgReportKpi = ({ x, y, width, label, value, caption, color }) => `
  <rect x="${x}" y="${y}" width="${width}" height="72" rx="10" fill="#fff" stroke="#dbe4ef"/>
  <rect x="${x + 14}" y="${y + 14}" width="5" height="22" rx="2.5" fill="${color}"/>
  <text x="${x + 28}" y="${y + 25}" font-size="8.4" font-weight="900" fill="#475569">${escapeXml(label)}</text>
  <text x="${x + 14}" y="${y + 50}" font-size="17.4" font-weight="900" fill="#071936">${escapeXml(value)}</text>
  <text x="${x + 14}" y="${y + 65}" font-size="7.1" font-weight="850" fill="${color}">${escapeXml(caption)}</text>
`;
