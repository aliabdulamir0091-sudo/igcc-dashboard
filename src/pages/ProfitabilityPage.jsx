...
const svgReportKpi = ({ x, y, width, label, value, caption, color, icon }) => `
  <rect x="${x}" y="${y}" width="${width}" height="76" rx="10" fill="#fff" stroke="#dbe4ef"/>
  <rect x="${x + 14}" y="${y + 14}" width="26" height="26" rx="8" fill="${color}14" stroke="${color}44"/>
  <text x="${x + 27}" y="${y + 32}" text-anchor="middle" font-size="11" font-weight="900" fill="${color}">${escapeXml(icon)}</text>
  <text x="${x + 50}" y="${y + 27}" font-size="8.8" font-weight="900" fill="#475569">${escapeXml(label)}</text>
  <text x="${x + 14}" y="${y + 58}" font-size="19" font-weight="900" fill="#071936">${escapeXml(value)}</text>
  <text x="${x + width - 14}" y="${y + 58}" text-anchor="end" font-size="7.6" font-weight="850" fill="${color}">${escapeXml(caption)}</text>
`;
