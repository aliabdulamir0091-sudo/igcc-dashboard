...
const buildPnlReportSvg = ({ analysis, selectedCostCenter }) => {
  ...
  const bridgeItems = [
    { label: ["Gross Profit", "Before CN"], value: approved.grossProfit, color: "#2f6fb8" },
    { label: ["CN Impact"], value: cnImpact, color: "#16877a" },
    { label: ["Final Net Profit"], value: approved.netProfit, color: "#1b8177" },
  ];
  ...
  const insightLines = [
    `${selectedCostCenter || "Selected scope"} is ${isProfitable ? "profitable" : "loss-making"} on approved AFP with ${formatReportCurrency(approved.netProfit)} net profit.`,
    `${gapStatus} Submitted-basis net profit is ${formatReportCurrency(submitted.netProfit)} versus approved-basis ${formatReportCurrency(approved.netProfit)}.`,
    ...
    `Margin quality is ${approved.netMargin >= 20 ? "healthy" : approved.netMargin >= 0 ? "watch-listed" : "under pressure"} on the approved basis.`,
  ];
