...
const svgReportKpi = ({ x, y, width, label, value, caption, color, type = "profit" }) => `
  ...
  <text x="${x + 54}" y="${y + 28}" font-size="8.8" font-weight="900" fill="#475569">${escapeXml(label)}</text>
  <text x="${x + 14}" y="${y + 60}" font-size="17.8" font-weight="900" fill="#071936">${escapeXml(value)}</text>
  <rect x="${x + width - 70}" y="${y + 49}" width="56" height="16" rx="8" fill="${color}12"/>
  <text x="${x + width - 42}" y="${y + 60}" text-anchor="middle" font-size="6.8" font-weight="900" fill="${color}">${escapeXml(caption)}</text>
`;
