...
const buildPnlReportSvg = ({ analysis, selectedCostCenter }) => {
  ...
  const donutSegments = distributionRows.map((row) => {
    const segment = svgDonutSegment({ cx: 122, cy: 514, radius: 44, color: row.color, percent: row.share, offset: donutOffset });
    donutOffset += row.share;
    return segment;
  }).join("");
  const distributionLegend = distributionRows.map((row, index) => {
    const y = 478 + index * 21;
    return `<circle cx="202" cy="${y - 4}" r="4.5" fill="${row.color}"/><text x="214" y="${y}" font-size="8.2" font-weight="850" fill="#102033">${escapeXml(row.label.slice(0, 23))}</text><text x="374" y="${y}" text-anchor="end" font-size="8.2" font-weight="900" fill="#102033">${escapeXml(formatPercent(row.share))}</text>`;
  }).join("");
  ...
  const cnBars = cnRows.map((row, index) => {
    ...
    return `<text x="420" y="${y}" font-size="9.5" font-weight="850" fill="#102033">${escapeXml(`${row.mode} - ${row.label}`.slice(0, 34))}</text>${svgBar({ x: 420, y: y + 8, width: 150, value: row.amount, max: maxCn, color })}<text x="718" y="${y}" text-anchor="end" font-size="9.5" font-weight="900" fill="${color}">${escapeXml(formatReportCurrency(row.amount))}</text>`;
  }).join("");
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="794" height="1123" viewBox="0 0 794 1123"><rect width="794" height="1123" fill="#fff"/><rect x="28" y="28" width="738" height="1067" rx="16" fill="#fff" stroke="#dbe4ef"/><image href="${igccLogo}" x="54" y="55" width="70" height="70"/><rect x="144" y="52" width="414" height="84" rx="0" fill="#071936"/><path d="M496 52h62v84h-78c22-26 27-54 16-84Z" fill="#99f6e4"/><text x="170" y="83" font-size="25" font-weight="900" fill="#fff">PROFIT &amp; LOSS</text><text x="170" y="112" font-size="21" font-weight="900" fill="#8ee6d2">COST CENTER REPORT</text><text x="170" y="132" font-size="10.5" font-weight="850" fill="#fff">${escapeXml(selectedCostCenter || "Cost Center")} | ${escapeXml(analysis.print.meta.hub)} | ${escapeXml(analysis.print.meta.region)} | USD</text><rect x="570" y="52" width="170" height="84" fill="#0b2b52"/><text x="588" y="78" font-size="8.2" font-weight="800" fill="#cbd5e1">Report Date</text><text x="666" y="78" font-size="8.2" font-weight="900" fill="#fff">${escapeXml(analysis.print.meta.reportDate)}</text><text x="588" y="101" font-size="8.2" font-weight="800" fill="#cbd5e1">Period</text><text x="666" y="101" font-size="8.2" font-weight="900" fill="#fff">${escapeXml(reportPeriodLabel)}</text><text x="588" y="124" font-size="8.2" font-weight="800" fill="#cbd5e1">Currency</text><text x="666" y="124" font-size="8.2" font-weight="900" fill="#fff">USD</text>${svgReportKpi({ x: 54, y: 162, width: 160, label: "Status", value: isProfitable ? "Profitable" : "Loss-Making", caption: `${formatPercent(approved.netMargin)} net margin`, color: isProfitable ? "#16a34a" : "#dc2626" })}${svgReportKpi({ x: 224, y: 162, width: 160, label: "Adjusted Revenue", value: formatReportCurrency(approved.updatedRevenue), caption: `${revenueTrend >= 0 ? "Up" : "Down"} ${formatPercent(Math.abs(revenueTrend))} vs previous`, color: "#16a34a" })}${svgReportKpi({ x: 394, y: 162, width: 160, label: "Total Cost", value: formatReportCurrency(approved.updatedCost), caption: `${costTrend >= 0 ? "Up" : "Down"} ${formatPercent(Math.abs(costTrend))} vs previous`, color: "#ef4444" })}${svgReportKpi({ x: 564, y: 162, width: 176, label: "Net Profit", value: formatReportCurrency(approved.netProfit), caption: `${profitTrend >= 0 ? "Up" : "Down"} ${formatPercent(Math.abs(profitTrend))} | Margin ${formatPercent(approved.netMargin)}`, color: "#2563eb" })}${svgCard(54, 274, 336, 134, "1. Revenue & Cost Bridge", [["Approved AFP", approved.revenue, "#102033"], ["Issued Credit Notes", approved.issuedCreditNotes, "#16a34a"], ["Adjusted Revenue", approved.updatedRevenue, "#15803d"], ["Spent Report Cost", -approved.totalCost, "#dc2626"], ["Received Credit Notes", -approved.receivedCreditNotes, "#7c3aed"], ["Net Profit", approved.netProfit, approved.netProfit >= 0 ? "#15803d" : "#dc2626"]].map((row, index) => `<text x="74" y="${314 + index * 16}" font-size="9.5" font-weight="${index === 2 || index === 5 ? 900 : 750}" fill="#102033">${escapeXml(row[0])}</text><text x="354" y="${314 + index * 16}" text-anchor="end" font-size="9.5" font-weight="900" fill="${row[2]}">${escapeXml(formatReportCurrency(row[1]))}</text><line x1="74" y1="${320 + index * 16}" x2="354" y2="${320 + index * 16}" stroke="#e7edf5"/>`).join(""))}${svgCard(404, 274, 336, 134, "2. Executive Summary", `<text x="424" y="318" font-size="10" font-weight="900" fill="#102033">Performance</text><text x="424" y="335" font-size="9.5" fill="#334155">The cost center is ${isProfitable ? "profitable" : "loss-making"} with ${escapeXml(formatPercent(approved.netMargin))} net margin.</text><text x="424" y="362" font-size="10" font-weight="900" fill="#102033">Key Cost Driver</text><text x="424" y="379" font-size="9.5" fill="#334155">${escapeXml(topDriver?.glName || "Not identified")} contributes ${escapeXml(formatPercent(topDriver?.share || 0))} of total cost.</text><text x="424" y="399" font-size="10" font-weight="900" fill="#102033">Action Required</text><text x="520" y="399" font-size="9.5" fill="#334155">Review high-value GL and CN drivers.</text>`)}${svgCard(54, 430, 336, 150, "3. Cost Distribution (Visual)", `<circle cx="122" cy="514" r="44" fill="none" stroke="#e8edf5" stroke-width="28"/>${donutSegments}<circle cx="122" cy="514" r="24" fill="#fff"/>${distributionLegend}`)}${svgCard(404, 430, 336, 150, "4. Credit Note Breakdown", cnBars, `Impact ${formatReportCurrency(cnImpact)}`)}${svgCard(54, 604, 686, 150, "5. Profitability Movement", `<line x1="122" y1="710" x2="608" y2="710" stroke="#dbe4ef"/><line x1="166" y1="676" x2="320" y2="676" stroke="#94a3b8" stroke-dasharray="4 4"/><line x1="388" y1="676" x2="542" y2="676" stroke="#94a3b8" stroke-dasharray="4 4"/>${bridgeBars}`)}${svgCard(54, 774, 686, 150, "6. Executive Insights (Auto-Generated)", insights)}<text x="54" y="944" font-size="13" font-weight="900" fill="#071b6f">7. Strategic Actions</text>${actions}<text x="54" y="1072" font-size="9" font-weight="800" fill="#0f766e">Notes:</text><text x="92" y="1072" font-size="9" fill="#334155">AFP: Authorized Funding Program | CN: Credit Notes | pp: Percentage Points</text></svg>`;
};
